 ...
 version=`grep -Po '(?<=<Version>)([\.\d]+)(?=</Version>)' src/FSLib.Extension.csproj | head -n 1`.0
...
sed -i -r "s/oldVersion=\"[0-9\.-]+\"/oldVersion=\"1.0.0.0-$version\"/" src/content/App.config.install.xdt
